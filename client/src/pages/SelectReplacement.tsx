...
export default function SelectReplacement() {
  ...
  const incoming: Player | null = location.state?.incomingPlayer ?? null
  ...
  useEffect(() => {
    ...
    const loadTeam = async () => {
      ...
      try {
        ...
        const players: Player[] = res.data.players.map((p: any) => ({
        ...
        console.error(err)
        setError('Failed to load current squad')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleReplace = async (outgoingId: string) => {
    setError(null)
    setSubmitting(true)
    try {
      await api.patch('/team/transfer', {
        player_out_id: outgoingId,
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div className="px-4 text-sm text-gray-500">Loading squad...</div>
      ) : (
        POSITIONS.map(pos => {
          ...
          return (
            <div key={pos} className="mb-4 px-4">
              <p className="text-xs font-bold mb-2" style={{ color: positionColors[pos] }}>
                {pos}
              </p>
