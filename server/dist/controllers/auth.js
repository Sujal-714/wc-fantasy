...
exports.updatePassword = exports.updateUsername = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    ...
    try {
        ...
        const result = await db_1.default.query(`INSERT INTO users (email, username, password_hash)
       VALUES ($1, $2, $3)
       RETURNING id, email, username, created_at`, [email, username, password_hash]);
        const user = result.rows[0];
        // Create JWT
        const token = jsonwebtoken_1.default.sign({ userId: user.id }, // payload — what you want to store in the token
        config_1.config.jwt_secret, { expiresIn: '7d' } // token expires in 7 days
        );
...
const login = async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ userId: user.id }, config_1.config.jwt_secret, { expiresIn: '7d' });
...
const updateUsername = async (req, res) => {
    ...
    try {
       ...
       SET username = $1 
       WHERE id = $2 
       RETURNING id, email, username, created_at`, [username.trim(), userId]);
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        // Update password
        const updateResult = await db_1.default.query(`UPDATE users 
       SET password_hash = $1 
       WHERE id = $2 
       RETURNING id, email, username, created_at`, [password_hash, userId]);
