...
const transferPlayer = async (req, res) => {
    ...
    try {
        ...
        const teamResult = await client.query('SELECT id, budget_remaining FROM teams WHERE user_id = $1', [userId]);
        ...
        const playerOutResult = await client.query(`SELECT tp.purchase_price, tp.is_on_bench,tp.is_captain,
 p.position, p.name, p.is_injured
       FROM team_players tp
       JOIN players p ON tp.player_id = p.id
       WHERE tp.team_id = $1 AND tp.player_id = $2`, [team.id, player_out_id]);
        ...
        const playerInResult = await client.query('SELECT id, name, position, price FROM players WHERE id = $1', [player_in_id]);
        ...
        const alreadyInTeam = await client.query('SELECT id FROM team_players WHERE team_id = $1 AND player_id = $2', [team.id, player_in_id]);
        ...
        // Remove old player
        await client.query('DELETE FROM team_players WHERE team_id = $1 AND player_id = $2', [team.id, player_out_id]);
   ...
   VALUES ($1, $2, $3, $4, $5)`, [team.id, player_in_id, playerOut.is_captain, playerOut.is_on_bench, playerIn.price]);
        // Update budget
        await client.query('UPDATE teams SET budget_remaining = $1 WHERE id = $2', [newBudget, team.id]);
