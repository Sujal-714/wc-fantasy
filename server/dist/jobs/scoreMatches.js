...
exports.scoreMatches = void 0;
const db_1 = __importDefault(require("../db"));
const calculatePoints_1 = require("../utils/calculatePoints");
const scoreMatches = async () => {
    console.log('Scoring job started...');
    const client = await db_1.default.connect();
    try {
        // Find finished matches that have no stats yet
        const matchesResult = await client.query(`SELECT m.id, m.api_match_id
       FROM matches m
...
const scoreMatch = async (client, matchId) => {
    // Get all players whose country is playing in this match
    const match = await client.query('SELECT home_team, away_team FROM matches WHERE id = $1', [matchId]);
    const { home_team, away_team } = match.rows[0];
    const playersResult = await client.query(`SELECT id, position FROM players
     WHERE country = $1 OR country = $2`, [home_team, away_team]);
    ...
    try {
        for (const player of playersResult.rows) {
            ...
            const points = (0, calculatePoints_1.calculatePoints)(mockStats, player.position);
            await client.query(`INSERT INTO player_match_stats 
   ...
   ON CONFLICT (player_id, match_id) DO NOTHING`, [
        ...
        await client.query(`UPDATE teams t
       SET total_points = (
