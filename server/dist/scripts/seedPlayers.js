...
const seed = async () => {
    ...
    try {
        ...
        for (const player of players) {
            await client.query(`INSERT INTO players (api_player_id, name, position, country, price, is_injured)
         VALUES ($1, $2, $3, $4, $5, false)`, [player.api_player_id, player.name, player.position, player.country, player.price]);
        }
        await client.query('COMMIT');
        console.log(`✓ Seeded ${players.length} players successfully`);
        // Print a summary by country
        const summary = await client.query(`SELECT country, COUNT(*) as count 
       FROM players 
       GROUP BY country 
       ORDER BY country`);
        console.log('\nPlayers by country:');
